...
incident types which included. In 2015, these fire incident types represented
approximately `r RoundThousand(fire.calls). The following table shows these 
types sorted from most to least frequent.
...
It's reasonable to expect that the department's with the largest number of calls
are also the areas with the largest populations. Below is a list of 100 agencies
with the most reported calls. If you live in a highly populated area where the 
...
- Top 10 agencies - by call? by fire? by medical?

## Medical percentage


`r RoundThousand(medical.calls)`
